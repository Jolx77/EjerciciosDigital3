...
uint8_t PerdonFerPorTantoGlobal = 0; //Sirve para avisar que hay que esperar otros 5 ms x estar en alto
...
void configTimer1(){
	...
	mat1Cfg.MatchValue = ((((5*10^-3))/(5001))*(50*10^6))-1;
	mat1Cfg.ResetOnMatch = ENABLE;
	mat1Cfg.StopOnMatch = DISABLE;
	ConfigStructInit(TIM_TIMER_MODE,(void*) &tim1Cfg);
...
void TIMER1_IRQHandler(){
	if(PerdonFerPorTantoGlobal == 1){
		PerdonFerPorTantoGlobal = 0;
		counter++;
...
void EINT0_IRQHandler(){
	...
	EXTI_ClearEXTIFlag(EXTI_EINT0);
	EXTI_SetPolarity(EINT3_IRQn,EXTI_POLARITY_HIGH_ACTIVE_OR_RISING_EDGE);
	}
	else{
		polaridad = 0;
